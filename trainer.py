...
def case_study(model,
    ...
    if qr_only:
        ...
        inputs_qr, masks_qr = functions.preprocessing_for_bert_qa(X_qr)
        inputs_qr = inputs_qr.to(device)
        masks_qr = masks_qr.to(device)
        model.eval()
        with torch.no_grad():
            results = model(inputs_qr, masks_qr)
        ...
        X_qr = data.Question.values + data.Answer.values
        inputs_p, masks_p = functions.preprocessing_for_bert_p(X_p)
        inputs_qr, masks_qr = functions.preprocessing_for_bert_qa(X_qr)
        ...
        model.eval()
        with torch.no_grad():
            results = model(inputs_p, masks_p, inputs_qr, masks_qr)
        preds = torch.argmax(results, dim=1).flatten()
